...
from controllers.income_controller import add_income
import datetime
...
class IncomeView(tk.Frame):
    def __init__(self, parent, controller=None):
        ...
        tk.Label(self, text="Джерело:").grid(row=2, column=0, sticky='e')
        sources = ["їжа", "комунальні витрати", "транспорт", "розваги", "інше"]
        self.source_var = tk.StringVar(value=sources[0])
        tk.OptionMenu(self, self.source_var, *sources).grid(row=2, column=1, sticky='we')
        ...
        tk.Button(self, text="Додати надходження", command=self.submit).grid(row=4, column=0, columnspan=2, pady=5)
    ...
    def submit(self):
        ...
        add_income(d, amt, src, det)
        messagebox.showinfo("Успіх", "Надходження додано")
